...
Chúng tôi tiến hành nghiên cứu, đo đạc và ghi lại kết quả của việc thử nghiệm các hướng cải thiện khác nhau với mô hình kết hợp. Kết quả được ghi nhận tại các bảng 4.4, 4.5, 4.6, 4.7 và 4.8.

% Ablation study inliers
\subsection{Ablation study với các ngưỡng cặp điểm tương quan}
Chúng tôi tiến hành thực hiện những thí nghiệm nhằm nghiên cứu những ảnh hưởng cũng như những đánh đổi về mặt kết quả của mô hình kết hợp ở các ngưỡng tương quan khác nhau trên hai tập dữ liệu Pittsburgh250k-test \cite{6618963} và Cambridge Landmark \cite{kendall2016posenet}. Kết quả được thể hiện ở bảng 4.4.
% table
\bgroup
\def\arraystretch{1.4}%
\setlength\tabcolsep{10 pt}
...
\egroup
\subsubsection*{Nhận xét}
Do tính chất đa dạng góc quay của tập dữ liệu, việc thực hiện reranking danh sách ảnh truy vấn được không cải thiện được hiệu quả mô hình, ngược lại, có thể làm cho sai số tăng nhẹ. Điều này có thể được lý giải bởi hai lý do: thứ nhất, không có quá nhiều khác biệt giữa trọng số cơ sở và trọng số đã được tinh chỉnh, thứ hai do bản chất của tập dữ liệu có ít ảnh trùng nhau giữa cơ sở dữ liệu và tập dữ liệu truy vấn - tức ngoài ảnh đầu tiên được truy xuất là chính xác thì các ảnh sau chỉ làm cho kết quả bị sai lệch chứ không cải thiện.

% Ablation study Weighted Pose
\subsection{Ablation study với trung bình trọng số trên mô hình kết hợp}
Kết quả mô hình kết hợp khi áp dụng trung bình trọng số trên tập dữ liệu Pittsburgh250k-test \cite{6618963} được chúng tôi đo đạc tại bảng 4.7.
% table
\bgroup
\def\arraystretch{1.4}%
\setlength\tabcolsep{10 pt}
...
\egroup
\subsubsection*{Nhận xét}
Chúng tôi nhận thấy rằng với một tập dữ liệu đa dạng về góc nhìn tại một vị trí, việc tăng số ảnh truy xuất từ VPR làm giảm đi hiệu quả tương đối, thể hiện qua việc sai số vị trí và góc quay đều tăng sau khi tăng số ảnh truy xuất. Cũng như khi áp dụng kỹ thuật reranking, việc tăng số ảnh truy xuất gây ra hiện tượng các ảnh sai lệch được tính toán ở tác vụ RPR và dẫn đến sai số tăng hơn.
...
\egroup
\subsubsection*{Nhận xét}
Kết quả cho thấy hướng tiếp cận này vẫn không khả quan, có thể được giải thích bởi: mô hình cơ sở vốn đã có thể chọn ảnh tốt nhất cả về vị trí lẫn độ trùng lặp, hoặc ảnh có độ trùng lặp cao được chọn nhưng tác vụ RPR lại không thể tính toán tư thế chính xác do sai lệch ở các mô hình tiền huấn luyện ở tác vụ này.

\section{Kết luận}
Chúng tôi đã có thể đo đạc và so sánh kết quả các mô hình khác nhau với phương pháp đề xuất của nhóm. Chúng tôi rút ra được một số nhận xét tổng thể về điểm mạnh và điểm yếu cần cải thiện của giải pháp đề xuất. Về điểm mạnh, mô hình đề xuất của chúng tôi kết hợp được độ tổng quát hóa cao của VPR và độ chính xác của RPR trong tác vụ hồi quy tư thế, dẫn đến kết quả tốt hơn hết trong tác vụ Visual localization trên dữ liệu chưa được huấn luyện. Việc sử dụng mô hình cơ sở giúp chúng tôi giảm thiểu tài nguyên cũng như thời gian huấn luyện mô hình. Đồng thời, việc sử dụng các mô hình cơ sở cho tác vụ RPR giúp mô hình hoạt động tốt với nhiều tập dữ liệu khác nhau so với các mô hình học sâu. Thêm vào đó, việc áp dụng các ngưỡng tương quan và chiến lược khai phá mới đã giúp chúng tôi tiếp tục cải thiện hiệu quả của mô hình.

Về các điểm cần cải thiện, việc sử dụng các mô hình cơ sở được huấn luyện sẵn tiềm ẩn nhiều rủi ro lỗi tích lũy qua các bước của mô hình, dẫn đến sai số lớn ở kết quả đầu ra. Các mô hình feature matching thường hoạt động tốt trong môi trường trong nhà, hoặc với tập ảnh có một vật thể cố định nào đó là cột móc thay vì toàn cảnh đô thị. Ngoài ra, thời gian thực thi của nhiều mô hình cơ sở sẽ làm tăng thời gian thực thi của toàn thể mô hình kết hợp. Cuối cùng, các hướng tiếp cận với tiềm năng cải thiện pipeline ban đầu phần nhiều vẫn chưa thể hiện ảnh hưởng tích cực đến độ hiệu quả của mô hình trên dữ liệu thành thị phạm vi rộng và thưa.
